...
function getcategoriesmenu_v2($table)
	{
		require("database/db_connect.php");
		$sql="SELECT * FROM $table ";
		if ($result=mysqli_query($con,$sql))
		{
			...
			<a class="fw-bolder mb-1" style="color:#000;font-weight:600" href="category.php?id='.$category['id'].'">'.$category['name'].'</a>
			<span class="badge badge-success">'.$rowcountcategory.'</span>
			</li>
...
function getbottomsliderposts_v2($table){
	require("database/db_connect.php");
...
function getblogridposts_v2($table){
	require("database/db_connect.php");
      	...
      	# code...
			...
			<div class="col-md-5 blog-grid-left">
				<a href="single.php?id='.$griditem['id'].'">
					<img src="blogadmin/images/'.$griditem['photo'].'" class="img-fluid" alt="fantastic cms">
				</a>
			</div>
...
function getblogridposts($table){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE posted='publish' ORDER BY id DESC LIMIT 6";
	if ($result=mysqli_query($con,$sql))
	{
      	//count number of rows in query result
		$rowcount=mysqli_num_rows($result);
		...
		foreach ($result as $bloggrid => $griditem) {
      	# code...
			echo '<div class="col-sm-4 blog-grid-top">
			<div class="b-grid-top">
			<div class="blog_info_left_grid">
			<a href="single.php?id='.$griditem['id'].'">
			<img src="blogadmin/images/'.$griditem['photo'].'" class="img-fluid" alt="fantastic cms" style="width:350px;height:250px;object-fit:cover;object-position:center;">
			</a>
			</div>
			<h3>
			<a href="single.php?id='.$griditem['id'].'">'.$griditem['title'].'</a>
...
function getblogridposts_single_post($table){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE posted='publish' ORDER BY id DESC LIMIT 3";
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $edschoice => $choice) {
			...
			if ($result=mysqli_query($con,$sql)) {
				...
				foreach ($result as $posts => $postdata) {
					...
					<img src="blogadmin/images/'.$postdata['photo'].'" class="img-fluid" alt="fantastic cms" style="width:350px;height:250px;object-fit:cover;object-position:center;">
					</a>
					</div>
					<br>
					<a href="#" style="color:#000" class="medium text-muted">'.$postdata['date'].'</a>
					<h2>
					<a style="color:#000;font-size:0.6cm;font-weight:bold" href="single.php?id='.$postdata['id'].'">'.$postdata['title'].'</a>
					</h2>
					<br>
					<a href="single.php?id='.$postdata['id'].'" class="btn btn-primary read-m">Selengkapnya</a>
...
function geteditorschoice_v2($table){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table ORDER BY id DESC LIMIT 4";
	if ($result=mysqli_query($con,$sql))
	{
...
function getcategoryblogs($table,$id){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE category='$id' ORDER BY id DESC LIMIT 1000";
	if ($result=mysqli_query($con,$sql))
	{
      	//count number of rows in query result
		$rowcount=mysqli_num_rows($result);
      	...
      	# code...
			echo '<div class="col-md-6 card">
							...
							<div class="card-body">
								...
								<a href="single.php?id='.$cdata['id'].'" class="btn btn-success read-m">Selengkapnya</a>
							</div>
						</div>';
		}
	}
...
function getcategoryblogs_v2($table,$id){
	require("database/db_connect.php");
	$sql="SELECT * FROM $table WHERE category='$id' ORDER BY id DESC LIMIT 1000";
	if ($result=mysqli_query($con,$sql))
	{
      	...
      	# code...
			echo '<div class="col-md-6 card">
							...
							<h5 class="card-title ">
								...
								<a href="single.php?id='.$cdata['id'].'" class="btn btn-primary read-m">Selengkapnya</a>
							</div>
						</div>';
		}
	}
